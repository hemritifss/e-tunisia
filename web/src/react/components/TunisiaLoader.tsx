...
export default function TunisiaLoader({ size = 52, label, fullscreen = false, className = '' }: TunisiaLoaderProps) {
  const mark = (
    ...
    >
      <path className="tn-loader-shape" pathLength={1} transform={TN_TRANSFORM} d={TN_PATH} />
    </svg>
  );
  const inner = (
    <>
      {mark}
      {label ? <span className="tn-loader-label">{label}</span> : null}
    </>
  );
  if (fullscreen) {
    return <div className={`tn-loader tn-loader--full ${className}`.trim()} role="status" aria-live="polite">{inner}</div>;
  }
  return <span className={`tn-loader ${className}`.trim()} role="status" aria-live="polite">{inner}</span>;
}
