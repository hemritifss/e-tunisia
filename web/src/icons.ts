// ============================================
// SVG ICON SYSTEM
// Inline SVGs for reliability (no CDN dependency)
// ============================================
...
const icons: Record<string, string> = {
  ...
  'chevron-up': '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m18 15-6-6-6 6"/></svg>',
  'chevron-down': '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>',
  ...
  'footprints': '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 16v-2.38C4 11.5 2.97 10.5 3 8c.03-2.72 1.49-6 4.5-6C9.37 2 10 3.8 10 5.5 10 7.31 9 12 4 16Z"/><path d="M16 8c2.5 0 4 1.34 4 4.5S18 18 16 20"/><path d="M12 20c-2.5 0-4-1.34-4-4.5S10 10 12 8"/></svg>',
  ...
  'at-sign': '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="4"/><path d="M16 8v5a3 3 0 0 0 6 0v-1a10 10 0 1 0-4 8"/></svg>',
  ...
  'file-question': '<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 17h.01"/><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7z"/><path d="M9.1 9a3 3 0 0 1 5.82 1c0 2-3 3-3 3"/></svg>',
};

/**
 * Replace all <i class="lucide-xxx"> elements with inline SVGs
 */
export function replaceIcons(root: HTMLElement | Document = document) {
  const iconElements = root.querySelectorAll('i[class*="lucide-"]');
  iconElements.forEach(el => {
    const classes = el.className.split(' ');
    const iconClass = classes.find(c => c.startsWith('lucide-'));
    if (!iconClass) return;
    ...
    if (svg) {
      ...
      span.innerHTML = svg;
      span.className = classes.filter(c => !c.startsWith('lucide-')).join(' ');
