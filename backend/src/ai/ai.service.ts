...
/** The concierge's human name — change here to rebrand the persona everywhere. */
...
export class AIService {
  ...
  async chatTravelPlanner(
    ...
    premium = false,
    userCtx?: { fullName?: string; interests?: string[]; favoriteIds?: string[]; visitedPlaceIds?: string[] },
  ): Promise<{ reply: string; suggestions?: string[]; places?: GroundedPlace[] }> {
    if (!this.llm.live) {
      return this.generateMockChatResponse(messages);
    }

    // Personalize the persona with who we're actually talking to (name, tastes,
    // places they've already saved) so replies feel like they *know* the user.
    let system = CONCIERGE_SYSTEM;
    if (userCtx) {
      let savedPlaces: string[] = [];
      try {
        const favs = (userCtx.favoriteIds || []).slice(0, 8);
        if (favs.length) savedPlaces = (await this.placesService.getByIds(favs)).map((p) => p.name).filter(Boolean).slice(0, 6);
      } catch { /* personalization is best-effort — never block the chat */ }
    ...
    try {
      ...
      // The model ends its reply with a hidden `SUGGESTIONS:` line — split the real
      // follow-ups out of the visible text so the chips are contextual, not canned.
  ...
  /** Build the "who am I talking to" block appended to the concierge system prompt. */
  private buildPersonaContext(profile: {
    firstName?: string;
    interests: string[];
    savedPlaces: string[];
    visitedCount: number;
    ...
    const bits: string[] = [];
    if (profile.firstName) bits.push(`Their name is ${profile.firstName} — use it naturally now and then, not in every message.`);
    if (profile.interests.length) bits.push(`They're into: ${profile.interests.join(', ')}. Lean your tips this way.`);
    if (profile.savedPlaces.length) bits.push(`Places they've already saved/loved: ${profile.savedPlaces.join(', ')}. Reference these to make recommendations feel personal (e.g. "since you liked …").`);
    if (profile.visitedCount) bits.push(`They've explored around ${profile.visitedCount} spot(s) so far — treat them as someone who already knows a bit of Tunisia.`);
    if (!bits.length) return '';
    return `\n\nWHO YOU'RE TALKING TO (never read this back verbatim, just let it shape you):\n- ${bits.join('\n- ')}`;
  }

  /**
   * Split the model's reply from the trailing `SUGGESTIONS: a | b | c` line.
   * Everything from the marker onward is stripped from the visible reply.
   */
  private splitSuggestions(text: string): { reply: string; suggestions: string[] } {
    const raw = (text || '').trim();
    const m = raw.match(/SUGGESTIONS?\s*:\s*(.+)\s*$/im);
    if (!m || m.index === undefined) return { reply: raw, suggestions: [] };
