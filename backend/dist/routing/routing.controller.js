"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RoutingController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const routing_service_1 = require("./routing.service");
let RoutingController = class RoutingController {
    constructor(routing) {
        this.routing = routing;
    }
    route(coords) {
        return this.routing.route(this.routing.parseCoords(coords));
    }
    optimize(coords) {
        return this.routing.optimize(this.routing.parseCoords(coords));
    }
};
exports.RoutingController = RoutingController;
__decorate([
    (0, common_1.Get)('route'),
    (0, swagger_1.ApiOperation)({ summary: 'Driving route (road geometry + per-leg distance/duration)' }),
    __param(0, (0, common_1.Query)('coords')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], RoutingController.prototype, "route", null);
__decorate([
    (0, common_1.Get)('optimize'),
    (0, swagger_1.ApiOperation)({ summary: 'Optimize stop order for shortest drive (TSP), returns route' }),
    __param(0, (0, common_1.Query)('coords')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], RoutingController.prototype, "optimize", null);
exports.RoutingController = RoutingController = __decorate([
    (0, swagger_1.ApiTags)('routing'),
    (0, common_1.Controller)('routing'),
    __metadata("design:paramtypes", [routing_service_1.RoutingService])
], RoutingController);
//# sourceMappingURL=routing.controller.js.map